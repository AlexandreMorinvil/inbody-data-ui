...
- 🎯 Entry points to execute your program using `python -m <inbody_data_ui>` or `$ inbody_data_ui` with basic CLI argument parsing.
...
---
# inbody_data_ui Flask Application

Awesome inbody_data_ui created by AlexandreMorinvil
...
```bash
git clone https://github.com/AlexandreMorinvil/inbody-data-ui inbody_data_ui
cd inbody_data_ui
make install
```
...
```bash
inbody_data_ui create-db   # run once
inbody_data_ui populate-db  # run once (optional)
inbody_data_ui add-user -u admin -p 1234  # ads a user
inbody_data_ui run
```
